 ...
 *
 * A short budget takes objectives spaced through the lecture, first and last
 * included, so the session spans the lecture rather than stopping partway.
 *
...
export function planNextTurn(
  ...
  const chosen = spacedSubset(active, options.los);
